...
if (isset($_POST['cari'])) {
    ...
    $cari = $_POST['cari'];
    $data = mysqli_query($koneksi, "SELECT * FROM playlists WHERE status=1 AND nama LIKE '%$cari%'");
    $jumlah_data = mysqli_num_rows($data);
    $total_halaman = ceil($jumlah_data / $batas);

    $playlist_query = mysqli_query($koneksi, "SELECT * FROM playlists WHERE status=1 AND nama LIKE '%$cari%' LIMIT $halaman_awal, $batas");
}
?>
...
<div class="row d-flex justify-content-between">
    <div class="col-lg-12 col-md-12">
        <div class="section-tittle mb-30">
            <h3><a href="index.php">Daftar Playlist</a> <i class="fa fa-fw fa-angle-double-right"></i> Cari Playlist
                "<?= $cari ?>"</h3>
        </div>
    </div>
</div>
<div class="blog_left_sidebar">
    ...
    <nav class="blog-pagination justify-content-center d-flex">
        <ul class="pagination">
            <li class="page-item">
                <a href="<?= ($halaman > 1) ? 'index.php?user=cari&halaman=' . $previous : '' ?>" class="page-link"
                    aria-label="Previous">
            ...
            <li class="page-item">
                <a href="index.php?user=cari&halaman=<?= $x ?>" class="page-link"><?= $x ?></a>
            </li>
            <?php } ?>

            <li class="page-item">
                <a href="<?= ($halaman < $total_halaman) ? 'index.php?user=cari&halaman=' . $next : '' ?>"
