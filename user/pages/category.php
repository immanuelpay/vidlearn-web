...
if (isset($_GET['kategori'])) {
    $id_category = $_GET['kategori'];
    $kategori_query = mysqli_query($koneksi, "SELECT * FROM categories WHERE status=1 AND id=$id_category");
    ...
    $data = mysqli_query($koneksi, "SELECT * FROM playlists WHERE status=1 AND id_category=$id_category");
    $jumlah_data = mysqli_num_rows($data);
    $total_halaman = ceil($jumlah_data / $batas);

    $playlist_query = mysqli_query($koneksi, "SELECT * FROM playlists WHERE status=1 AND id_category=$id_category LIMIT $halaman_awal, $batas");
}
?>
...
<div class="row d-flex justify-content-between">
    <div class="col-lg-12 col-md-12">
        <div class="section-tittle mb-30">
            <h3><a href="index.php">Daftar Playlist</a> <i class="fa fa-fw fa-angle-double-right"></i> Kategori
                <?= $kategori['nama'] ?></h3>
        </div>
    </div>
</div>
<div class="blog_left_sidebar">
    ...
    <nav class="blog-pagination justify-content-center d-flex">
        <ul class="pagination">
            <li class="page-item">
                <a href="index.php?user=kategori&kategori=<?= $id_category ?><?= ($halaman > 1) ? '&halaman=' . $previous : '' ?>"
            ...
            <li class="page-item">
                <a href="index.php?user=kategori&kategori=<?= $id_category ?>&halaman=<?= $x ?>"
                    class="page-link"><?= $x ?></a>
            </li>
            <?php } ?>

            <li class="page-item">
                <a href="index.php?user=kategori&kategori=<?= $id_category ?><?= ($halaman > 1) ? '&halaman=' . $next : '' ?>"
