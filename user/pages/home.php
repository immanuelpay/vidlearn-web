...
$data = mysqli_query($koneksi, "SELECT * FROM playlists WHERE status=1");
$jumlah_data = mysqli_num_rows($data);
$total_halaman = ceil($jumlah_data / $batas);

$playlist_query = mysqli_query($koneksi, "SELECT * FROM playlists WHERE status=1 ORDER BY nama ASC LIMIT $halaman_awal, $batas");
?>
...
<div class="blog_left_sidebar">
    ...
    <nav class="blog-pagination justify-content-center d-flex">
        <ul class="pagination">
            <li class="page-item">
                <a href="<?= ($halaman > 1) ? '?halaman=' . $previous : '' ?>" class="page-link" aria-label="Previous">
                    <i class="ti-angle-left"></i>
                </a>
            </li>
            <?php for ($x = 1; $x <= $total_halaman; $x++) { ?>
            <li class="page-item">
                <a href="?halaman=<?= $x ?>" class="page-link"><?= $x ?></a>
            </li>
            <?php } ?>

            <li class="page-item">
                <a href="<?= ($halaman < $total_halaman) ? '?halaman=' . $next : '' ?>" class="page-link"
                    aria-label="Next">
